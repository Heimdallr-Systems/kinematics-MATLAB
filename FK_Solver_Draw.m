function FK_Solver_Draw(Theta1,Theta2,Theta3,BodyRot,rBfromI,setgndplane)
...
% Orientation of Body
 TB = rotz(jnt_var(13))*roty(jnt_var(14))*rotx(jnt_var(15));
...
FRLink1_v=repmat(r1_FR,1,length(FRLink1))+T1_FR*rotz(pi)*FRLink1';
FRLink2_v=repmat(r2_FR,1,length(FRLink2))+T2_FR*rotx(pi)*FRLink2';
FRLink3_v=repmat(r3_FR,1,length(FRLink3))+T3_FR*rotx(pi)*FRLink3';

BLLink1_v=repmat(r1_BL,1,length(BLLink1))+T1_BL*rotz(pi)*BLLink1';
BLLink2_v=repmat(r2_BL,1,length(BLLink2))+T2_BL*rotx(pi)*BLLink2';
BLLink3_v=repmat(r3_BL,1,length(BLLink3))+T3_BL*rotx(pi)*BLLink3';
