...
step_dist = 0.2; % leg step distance
...
for ii = 1:length(t)
    ...
    if (norm(r_II_B_d - r_II_B) >= max_body_dist)  || (step_state ~= 0) || (~goal_inside_pgon)
        ...
        if reached_rest_centroid == 0 % needs to move back to resting 4-legged position to find new leg to move
            ...
            [x,y] = centroid(pgon);
            r_II_B_d_temp = [x;y;r_II_B_d(3)];
            [Theta1_d,Theta1_2_d,Theta2_1_d,Theta2_2_d,Theta2_3_d,Theta2_4_d,Theta3_1_d,Theta3_2_d,Theta3_3_d,Theta3_4_d,r_II_B_d_temp] = Body_Pose_Controller(r_II_c, T_I_B_d,r_II_B_d_temp,r_II_B,legs_on_gnd);
        ...
        [Theta1_d,Theta1_2_d,Theta2_1_d,Theta2_2_d,Theta2_3_d,Theta2_4_d,Theta3_1_d,Theta3_2_d,Theta3_3_d,Theta3_4_d,r_II_B_d_temp] = Body_Pose_Controller(r_II_c, T_I_B_d,r_II_B_d,r_II_B,legs_on_gnd);
        % if theta1 wraps around into robot
        T1_cond(1) = (Theta1_d(1) <= -pi/2) || (Theta1_d(1) >= pi); % FR
        T1_cond(2) = (Theta1_d(2) <= -pi) || (Theta1_d(2) >= pi/2); %FL
        T1_cond(3) = (Theta1_d(3) <= -pi) || (Theta1_d(3) >= pi/2); % BR
    ...
    %         if (hh == 2) || (hh == 4)
    %             if Theta3_d(hh) > 0
    %                 Theta3_d(hh) = Theta3_1_d(hh);
    %                 Theta2_d(hh) = Theta2_1_d(hh);
    %             end
    %         else
    %             if Theta3_d(hh) < 0
    %                 Theta3_d(hh) = Theta3_1_d(hh);
